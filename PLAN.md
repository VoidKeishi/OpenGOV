...
2. **Golden QA ≥ 90%** — runner đã nâng cấp (session theo khối thủ tục như hội thoại thật, gate `GOLDEN_QA_MIN`, fix đọc payload card); pass-rate 63% → 70% qua 3 vòng tune (prompt v2.2 + 2 gap-fill legal + resolver tail, đo trên claude-sonnet-5). **Credit OpenRouter đã cạn 19/07** → model chính đổi sang `google/gemini-3.5-flash`, fallback `tencent/hy3:free`; **quy tắc: không chạy eval tự động khi chưa được duyệt ngân sách** — khi được duyệt (ưu tiên nạp thêm credit): chạy subset câu fail trước (rẻ), đạt rồi mới `GOLDEN_QA_MIN=90 BASE_URL=https://opengov.duckdns.org pnpm --dir backend golden-qa` làm evidence; lưu ý baseline 70% đo trên Sonnet — Flash cần đo lại. Xong → chấm nốt dòng ◐ acceptance §11 (manual replay kịch bản intake).
