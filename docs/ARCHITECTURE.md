...
- `update_case_facts(facts)` — see session memory. Each turn also re-injects the session's recorded `case_facts` as a trailing system note (history persists prose only), so follow-up turns never re-ask what is already stored; the assisted-fill prompt rule makes the model collect missing form facts one question per turn and hand back to the widget's confirmed-prefill button when done. A deterministic guard backs the rule up: an answer claiming "ghi nhận" in a turn with no `update_case_facts` call triggers one corrective pass (`facts_claim_without_call` warning) that forces the call — live-observed model behavior, prompt alone is not reliable.

System prompt enforces: answer only from tool results; short answers (~120 words, clarify-first on vague intent, questions sourced from the record's `case_facts_schema`); always cite legal-basis codes + source link; out-of-KB → explicit "không có trong dữ liệu" + portal link; markdown restricted to the widget's mini-markdown subset; **demo grounding** — the prompt states the widget runs ON the demo portal itself, so for the 3 pilot procedures the model gives on-site directions ("bấm Nộp trực tuyến", "sang bước Tờ khai") instead of sending users to dichvucong.gov.vn (which stays the citation source + fail-closed link for everything else). `maxTokens: 1200` on the agent's completion calls is the safety net; the OpenRouter client disables reasoning (`reasoning: {effort: "none"}` — every model in the chain is reasoning-capable and reasoning tokens count toward `max_tokens`, which truncated answers and the cards tail at 700) and warns when a response comes back with `finish_reason=length`. Structured numbers (fees, deadlines, agencies, freshness date) are emitted as card payloads read verbatim from the record; the LLM stream carries prose only.

The model steers the UI via a **directive tail**: trailing machine-readable lines the service parses and strips before the number guard, streaming, and history persistence (unknown `[[KEY:]]` lines are stripped silently — forward-compatible). Three directives: `[[CARDS: <code>=<types>]]` selects cards (fail-safe: `procedure` + `legal_fragments` of the last-visited code + a `cards_tail_missing` warning); `[[CHIPS: a | b]]` emits a `chips` SSE event (≤3 quick-reply suggestions, clamped, progressive enhancement); `[[GUIDE: <code>=<target>]]` emits a `guide` card after validating `target` against the procedure's schema field keys ∪ `{submit}` (invalid → dropped + `guide_target_unknown` warning). Selecting `checklist` builds the tick-able checklist card from the curated groups filtered against session case_facts (re-read after the turn; missing fact → keep item with `conditional: true`). Cards for procedures that have a form schema carry `form_path` (`OPENGOV_FORM_PATH_PREFIX`, default `/nop-truc-tuyen/`, + `form_ref`) so the widget can render on-portal CTAs; `GET /schemas` additionally serves each schema's `prefill` map + `form_path` for the widget's confirmed-prefill flow (DATA.md §4, WIDGET.md §12).
