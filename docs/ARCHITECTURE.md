...
- `get_procedure(code)` — full merged record (base + curated overlay) into context
- `get_form_schema(code)` — validation schema for the procedure
- `update_case_facts(facts)` — see session memory

System prompt enforces: answer only from tool results; always cite legal-basis codes + source link; out-of-KB → explicit "không có trong dữ liệu" + portal link. Structured numbers (fees, deadlines, agencies, freshness date) are emitted as card payloads read verbatim from the record; the LLM stream carries prose only.
...
- **Golden Q&A set as automated eval**: `backend/scripts/golden-qa.ts` runs 20–30 real-citizen questions through `/chat`, asserts expected procedure, must-mention phrases, and citation presence via normalized substring match (no LLM judge). Run on every prompt/model change — no tuning by feel.
