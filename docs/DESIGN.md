...
| **Ngân sách trả lời: prompt là kiểm soát chính (~120 từ), `maxTokens: 700` làm lưới an toàn** | Không giới hạn (hiện trạng) / chỉ maxTokens | Trả lời vừa một màn panel 380px, không đứt giữa câu (maxTokens gấp ~3 lần budget prompt); người dùng lowtech không bị tường chữ. Đổi lại: câu hỏi thật sự cần chi tiết phải hỏi thêm lượt nữa (chủ đích — hội thoại từng bước) |
