...
Đọc theo thứ tự: `PROBLEM.md` (đề bài + tiêu chí — bất biến, không sửa) → `PLAN.md` (tiến độ + việc còn lại) → `docs/DESIGN.md` (giải pháp, use case, quyết định thiết kế). Sau đó theo mảng đang làm:
...
- Quyết định thiết kế mới hoặc thay đổi → ghi vào `docs/DESIGN.md` §6 (kèm trade-off), không chỉ để trong lịch sử chat.
